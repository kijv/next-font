  ...
  visitImportDecl(importDecl: acorn.ImportDeclaration) {
    ...
    try {
      resolvedId = import.meta.resolve(
        removeQuerySuffix(
          this.remapImports[importDecl.source.value] || importDecl.source.value,
        ),
      );
    } catch {}
    ...
    if (
      resolvedId != null &&
      this.fontLoaders
        .map((fontLoader) => import.meta.resolve(fontLoader))
        .includes(resolvedId)
    ) {
        ...
        const { local, functionName } = (() => {
          switch (specifier.type) {
            case 'ImportSpecifier': {
              ...
              const functionName =
                imported.type === 'Identifier' ? imported.name : local.name;
              return {
                local,
                functionName,
              };

...
export const VERSION_TYPE = ['major', 'minor', 'patch', 'alpha', 'beta', 'canary'] as const
...
export const isVersionType = (versionType: string): versionType is VersionType => {
  return VERSION_TYPE.includes(versionType as VersionType)
}

export async function runBump(names: Set<string>, versionType?: VersionType, dryRun?: boolean) {
  // 1. List workspaces
  ...
  for (const workspace of workspaces) {
    try {
      ...
      const pkgJson = (await import(pkgJsonPath).then((mod) => mod.default || mod)) as PackageJson
  ...
  if (workspacesToBump.length === 0) {
    ...
    workspacesToBump = workspacePkgs.filter((pkg) => selected.includes(pkg.name))
  }
  ...
  for (const pkg of workspacesToBump) {
    ...
    if (!semverVersion) {
      throw new Error(`Failed to parse ${pkg.version} in ${pkg.name} as semver`)
    }
        ...
        if (semverVersion.prerelease.length === 0) {
          ...
          if (prereleaseType === vt) {
            ...
            if (
              ...
              throw new Error(`Previous version is ${prereleaseType}, so you can't bump to ${vt}`)
            }
          }
        }
        break
  ...
  const commitMsg = [`chore: release npm package${tagsToApply.length > 1 ? 's' : ''}`, tagsMessage]
...
export async function publishWorkspace(isNightly?: boolean, dryRun?: boolean) {
  ...
  for (const { pkgNameWithoutScope, scope, version } of tags) {
    ...
    if (!semverVersion) throw new Error(`Parse semver version failed ${version}`)

    const workspace = workspaces.find((w) => w.name === pkgName)
    if (!workspace) throw new Error(`Workspace not found for ${pkgName}`)

    const isAlpha = semverVersion.prerelease.some((p) => p.toString().includes('alpha'))
    const isBeta = semverVersion.prerelease.some((p) => p.toString().includes('beta'))
    const isCanary = semverVersion.prerelease.some((p) => p.toString().includes('canary'))
...
const getWorkspaces = async () => {
  ...
  if (pm?.name === 'bun') {
    try {
      ...
      const lockfile = (await import(path.join(root, 'bun.lock'))) as Bun.BunLockFile

      workspaces = Object.entries(lockfile.workspaces).map(([workspace, pkg]) => {
        return {
          name: pkg.name!,
    ...
    try {
      workspacesListText = await createCommand(`pnpm ls -r --depth -1 --json`).outputString()
    } catch {
